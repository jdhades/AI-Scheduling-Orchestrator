...
import { Incident, IncidentType } from '../../src/domain/aggregates/incident.aggregate';
...
describe('Scenario 5 - Auto-Repair Intergration (ProcessIncidentEvidenceHandler)', () => {
        ...
        moduleRef = await Test.createTestingModule({
            ...
            providers: [
                ...
                IncidentRepository,
                {
                    ...
                    useValue: {
                        findById: jest.fn().mockResolvedValue({ id: 'emp-123', companyId: 'comp-abc', name: 'John Doe' }),
                        findAllByCompany: jest.fn().mockResolvedValue([{ id: 'emp-123', companyId: 'comp-abc', name: 'John Doe' }]),
                    },
                },
                OcrService,
                {
    ...
    it('should process OCR, parse with LLM, and Validate the Incident', async () => {
        ...
        const incident = Incident.reportIncident('comp-abc', 'emp-123', IncidentType.MEDICAL_LEAVE);
        incident.attachEvidence('http://evidence.jpg');
        incident.commit(); // Clear events
        await incidentRepo.save(incident);

        // 2. Map Command
        const command = new ProcessIncidentEvidenceCommand(incident.id, 'emp-123', 'http://evidence.jpg');
