...
describe('GeminiConversationalService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get<GeminiConversationalService>(GeminiConversationalService);
    ...
    describe('processText()', () => {
        it('should correctly parse a valid JSON response from Gemini', async () => {
            ...
            const result = await service.processText('I want to swap my shift with +1234567890');

            expect(result.getIntent()).toBe('swap_shift');
            expect(result.getConfidence()).toBe(0.95);
            expect(result.getEntities()).toEqual({ targetEmployeePhone: '+1234567890', shiftId: 'shift-1' });
            expect(result.getRawText()).toBe('I want to swap my shift with +1234567890');
    ...
    describe('processAudio()', () => {
        ...
        it('should correctly process audio, convert to base64, and parse the valid JSON intent from Gemini', async () => {
            ...
            expect(globalFetchSpy).toHaveBeenNthCalledWith(1, 'https://api.twilio.com/audio.mp3', expect.objectContaining({
                headers: { Authorization: expect.stringContaining('Basic ') }
            }));

            // Gemini call
            expect(globalFetchSpy).toHaveBeenNthCalledWith(2, expect.stringContaining('gemini-1.5-pro'), expect.objectContaining({
                method: 'POST',
        ...
        it('should return unknown if Twilio audio download fails', async () => {
            ...
            const result = await service.processAudio('http://bad-url', 'audio/mp3', 'sid', 'token');
