...
function makeRule(overrides: {
    ...
    return SemanticRuleAggregate.create({
        ...
        ruleText: overrides.ruleText ?? 'Los empleados no pueden trabajar más de 8 horas seguidas',
...
describe('SemanticRuleAggregate', () => {
    ...
    describe('create() factory', () => {
        it('should create a rule with correct values', () => {
            ...
            expect(rule.getRuleText()).toBe('Los empleados no pueden trabajar más de 8 horas seguidas');
    ...
    describe('updateText()', () => {
        it('should update rule text and invalidate embedding', () => {
            ...
            rule.updateText('Nueva regla: descanso mínimo de 12 horas entre turnos consecutivos');
...
describe('ConflictResolutionEngine', () => {
    ...
    it('should prefer legal rule over semantic rule (same type)', () => {
        const legal = makeRule({ id: 'legal-1', priority: RulePriority.legal(), ruleType: RuleType.create('restriction') });
        const semantic = makeRule({ id: 'semantic-1', priority: RulePriority.semantic(), ruleType: RuleType.create('restriction') });
    ...
    it('should mark escalation when same priority and same blocking type conflict', () => {
        const r1 = makeRule({ id: 'r1', priority: RulePriority.legal(), ruleType: RuleType.create('restriction') });
        const r2 = makeRule({ id: 'r2', priority: RulePriority.legal(), ruleType: RuleType.create('restriction') });
    ...
    it('should sort rules by priority ascending (legal first)', () => {
        const pref = makeRule({ id: 'pref', priority: RulePriority.preference(), ruleType: RuleType.create('preference') });
        const legal = makeRule({ id: 'legal', priority: RulePriority.legal(), ruleType: RuleType.create('requirement') });
    ...
    it('should not escalate when there is no conflict', () => {
        const r1 = makeRule({ id: 'r1', priority: RulePriority.legal(), ruleType: RuleType.create('restriction') });
        const r2 = makeRule({ id: 'r2', priority: RulePriority.preference(), ruleType: RuleType.create('preference') });
        engine.resolveRules([r1, r2]);
