...
function makeValidLLMResponse(shiftId: string, employeeId: string): string {
    return JSON.stringify({
        assignments: [{ shiftId, employeeId, reason: 'Disponible y tiene la skill', confidence: 0.9 }],
    });
}
...
describe('PromptOrchestratorService', () => {
    ...
    it('should accept LLM proposals that pass validation', async () => {
        const llmResponse = JSON.stringify({
            assignments: [
                { shiftId: 's1', employeeId: 'e1', reason: 'Disponible', confidence: 0.9 },
                { shiftId: 's2', employeeId: 'e2', reason: 'Disponible', confidence: 0.85 },
            ],
        });

        const orchestrator = new PromptOrchestratorService(makeLLMMock(llmResponse), validator);
        const result = await orchestrator.orchestrate({
            employees, shifts, histories: [], companyId: 'company-1',
            weekStart: new Date('2026-03-04'), semanticRules: [],
        });
    ...
    it('should fall back to algorithm when LLM proposes non-existent employee', async () => {
        const invalidResponse = JSON.stringify({
            assignments: [
                { shiftId: 's1', employeeId: 'NON_EXISTENT', reason: 'OK', confidence: 0.95 },
            ],
        });

        const orchestrator = new PromptOrchestratorService(makeLLMMock(invalidResponse), validator);
        const result = await orchestrator.orchestrate({
            employees, shifts, histories: [], companyId: 'company-1',
            weekStart: new Date('2026-03-04'), semanticRules: [],
        });
    ...
    it('should fall back to algorithm when LLM returns empty response', async () => {
        const emptyLLM: jest.Mocked<ILLMService> = {
            complete: jest.fn().mockResolvedValue('No tengo propuestas para este schedule.'),
        };
    ...
    it('should filter out LLM proposals below confidence threshold (0.7)', async () => {
        const lowConfidenceResponse = JSON.stringify({
            assignments: [
                { shiftId: 's1', employeeId: 'e1', reason: 'No seguro', confidence: 0.5 }, // bajo threshold
                { shiftId: 's2', employeeId: 'e2', reason: 'Seguro', confidence: 0.9 },     // sobre threshold
            ],
        });

        const orchestrator = new PromptOrchestratorService(makeLLMMock(lowConfidenceResponse), validator);
        const result = await orchestrator.orchestrate({
            employees, shifts, histories: [], companyId: 'company-1',
            weekStart: new Date('2026-03-04'), semanticRules: [],
        });
    ...
    it('should reject LLM proposal when employee has time overlap with previous LLM assignment', async () => {
        ...
        const orchestrator = new PromptOrchestratorService(makeLLMMock(overlapResponse), validator);
        const result = await orchestrator.orchestrate({
            employees, shifts, histories: [], companyId: 'company-1',
            weekStart: new Date('2026-03-04'), semanticRules: [],
        });
