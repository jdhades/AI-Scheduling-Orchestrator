...
describe('CommandMapperService', () => {
    ...
    describe('swap_shift', () => {
        it('should map successfully when all entities are present', () => {
            ...
            const result = service.map(intent, 'empId', 'compId', {
                targetEmployeePhone: '+123',
                shiftId: 'shift-abc',
            });

            expect(result.command).toBeInstanceOf(SwapShiftCommand);
            const cmd = result.command as SwapShiftCommand;
            expect(cmd.requesterId).toBe('empId');
            expect(cmd.targetEmployeePhone).toBe('+123');
            expect(cmd.shiftId).toBe('shift-abc');
            expect(cmd.companyId).toBe('compId');
            expect(result.missingFields).toEqual([]);
        });

        it('should ask for clarification when both entities are missing', () => {
            const intent = ConversationIntentVO.create({
                intent: 'swap_shift',
                confidence: 0.9,
                entities: {},
                rawText: 'cambio turno',
            });

            const result = service.map(intent, 'empId', 'compId', {});

            expect(result.command).toBeNull();
            expect(result.missingFields).toEqual(['targetEmployeePhone', 'shiftId']);
            expect(result.clarificationMessage).toContain('1️⃣');
            expect(result.clarificationMessage).toContain('2️⃣');
        });
    });
    ...
    describe('request_day_off', () => {
        ...
        it('should default reason if omitted', () => {
            ...
            const result = service.map(intent, 'empId', 'compId', { date: '2026-03-10' });

            expect(result.command).toBeInstanceOf(RequestDayOffCommand);
            expect((result.command as RequestDayOffCommand).reason).toBe('No especificado');
        });
    ...
    describe('generate_schedule', () => {
        ...
        it('should map to specific date when specified', () => {
            ...
            const result = service.map(intent, 'empId', 'compId', { weekStart: '2026-03-16' });

            expect(result.command).toBeInstanceOf(GenerateHybridScheduleCommand);
            expect((result.command as GenerateHybridScheduleCommand).weekStart).toBe('2026-03-16');
        });
    });
