    ...
    describe('fromLLMResponse()', () => {
        ...
        it('should parse a valid JSON response with multiple assignments', () => {
            const raw = JSON.stringify({
                assignments: [
                    { shiftId: 'shift-1', employeeId: 'emp-1', reason: 'Tiene la skill', confidence: 0.95 },
                    { shiftId: 'shift-2', employeeId: 'emp-2', reason: 'Disponible', confidence: 0.8 },
                ],
            });
        ...
        it('should clamp confidence to [0, 1]', () => {
            const raw = JSON.stringify({
                assignments: [
                    ...
                    { shiftId: 's2', employeeId: 'e2', reason: 'Under', confidence: -0.3 },
                ],
            });
        ...
        it('should throw if no JSON block is found in response', () => {
            expect(() => LLMScheduleProposalVO.fromLLMResponse(
        ...
        it('should throw if JSON is malformed', () => {
            expect(() => LLMScheduleProposalVO.fromLLMResponse('{invalid json}')).toThrow(
                'LLMScheduleProposal: LLM response JSON is malformed',
            );
        });
        ...
        it('should filter out proposals below threshold', () => {
            const raw = JSON.stringify({
                assignments: [
                    ...
                    { shiftId: 's3', employeeId: 'e3', reason: 'Medium', confidence: 0.7 },
                ],
            });
            ...
            expect(filtered.getProposals().map(p => p.shiftId)).toEqual(['s1', 's3']);
        });
    });
});
