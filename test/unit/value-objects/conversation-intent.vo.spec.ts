...
describe('ConversationIntentVO', () => {
    describe('create()', () => {
        it('should create a valid intent with entities', () => {
            ...
            expect(intent.getEntities()).toEqual({ targetEmployeePhone: '+1234567890', shiftId: 'shift-1' });
        ...
        it('should throw DomainError if confidence is out of bounds', () => {
            expect(() => {
                ConversationIntentVO.create({ intent: 'swap_shift', confidence: -0.1, entities: {}, rawText: 'text' });
            }).toThrow(DomainError);

            expect(() => {
                ConversationIntentVO.create({ intent: 'swap_shift', confidence: 1.1, entities: {}, rawText: 'text' });
            }).toThrow(DomainError);
        });

        it('should throw DomainError if rawText is empty or whitespace', () => {
            expect(() => {
                ConversationIntentVO.create({ intent: 'swap_shift', confidence: 0.8, entities: {}, rawText: '' });
            }).toThrow(DomainError);
    ...
    describe('getMissingFields() & isComplete()', () => {
        it('should identify missing required fields', () => {
            const intent = ConversationIntentVO.create({
                intent: 'swap_shift',
                confidence: 0.9,
                entities: { shiftId: 'shift-1' }, // Missing targetEmployeePhone
                rawText: 'Swap this shift',
            });

            expect(intent.getMissingFields()).toEqual(['targetEmployeePhone']);
