...
describe('ConversationSessionVO', () => {
    ...
    describe('create()', () => {
        it('should create a valid session with correct TTL', () => {
            ...
            const expectedExpiresAt = new Date(now.getTime() + ConversationSessionVO.SESSION_TTL_SECONDS * 1000);
        ...
        it('should throw DomainError if employeePhone is missing', () => {
            expect(() => {
                ConversationSessionVO.create({ employeePhone: '', companyId: 'company-abc' });
            }).toThrow(DomainError);
        });

        it('should throw DomainError if companyId is missing', () => {
            expect(() => {
                ConversationSessionVO.create({ employeePhone: '+1234567890', companyId: '' });
            }).toThrow(DomainError);
        });
    });
    ...
    describe('withIntent() (merge entities)', () => {
        it('should return a new session instance with the intent and merged entities', () => {
            ...
            expect(newSession.getCollectedEntities()).toEqual({ targetEmployeePhone: '+0987654321' });
        ...
        it('should overwrite overlapping entities with new values', () => {
            ...
            const updatedSession = baseSession.withIntent('report_absence', { reason: 'Very sick' });
    ...
    describe('toSnapshot() & fromSnapshot()', () => {
        it('should serialize and deserialize preserving all data', () => {
            ...
            expect(restoredSession.getEmployeePhone()).toBe(session.getEmployeePhone());
            expect(restoredSession.getCompanyId()).toBe(session.getCompanyId());
            expect(restoredSession.getPendingIntent()).toBe(session.getPendingIntent());
            expect(restoredSession.getCollectedEntities()).toEqual(session.getCollectedEntities());
