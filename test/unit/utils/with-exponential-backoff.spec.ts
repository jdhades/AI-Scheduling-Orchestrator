...
describe('withExponentialBackoff', () => {
  ...
  it('should retry on HTTP 429 error and eventually succeed', async () => {
    ...
    const promise = await withExponentialBackoff(operation, 'TestCtx', {
      maxRetries: 3,
      initialDelayMs: 10,
    });
  ...
  it('should throw immediately if error is not rate limit or server error', async () => {
    const operation = jest
      .fn()
      .mockRejectedValue(new Error('Invalid input format (HTTP 400)'));

    await expect(withExponentialBackoff(operation, 'TestCtx')).rejects.toThrow(
  ...
  it('should throw after reaching max retries on rate limit', async () => {
    ...
    await expect(
      withExponentialBackoff(operation, 'TestCtx', {
        maxRetries: 2,
        initialDelayMs: 10,
      }),
