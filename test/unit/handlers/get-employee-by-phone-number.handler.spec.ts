...
describe('GetEmployeeByPhoneNumberHandler', () => {
    ...
    beforeEach(async () => {
        ...
        handler = module.get<GetEmployeeByPhoneNumberHandler>(GetEmployeeByPhoneNumberHandler);
    });

    it('should return EmployeeDto when employee is found', async () => {
        const phone = PhoneNumber.create('+1234567890');
        const exp = new ExperienceLevel(12, { junior: 6, intermediate: 24, senior: 999 });
    ...
    it('should throw NotFoundException when employee is not found', async () => {
        ...
        await expect(handler.execute(query)).rejects.toThrow('Employee with phone number +1234567890 not found.');
    });
});
