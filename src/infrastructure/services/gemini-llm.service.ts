...
export class GeminiLLMService implements ILLMService {
    ...
    private async callWithRetry(prompt: string, retriesLeft: number): Promise<string> {
        try {
            return await this.callGemini(prompt);
        } catch (error) {
            const isNetworkError = !(error instanceof Error && error.message.includes('HTTP'));
            if (retriesLeft > 0 && isNetworkError) {
                this.logger.warn(`GeminiLLMService: retrying after error: ${(error as Error).message}`);
    ...
    private async callGemini(prompt: string): Promise<string> {
        ...
        if (!response.ok) {
            throw new Error(`GeminiLLMService: HTTP ${response.status} — ${response.statusText}`);
        }
        ...
        this.logger.debug(`GeminiLLMService: received ${rawText.length} chars from Gemini`);
        return rawText;
    }
