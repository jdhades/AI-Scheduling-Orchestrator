...
export class GeminiEmbeddingService implements IEmbeddingService {
    ...
    async generate(text: string): Promise<number[]> {
        ...
        try {
            ...
            throw new Error(`EmbeddingService: network error — ${(networkError as Error).message}`);
        }
        ...
        if (!Array.isArray(values) || values.length === 0) {
            throw new Error('EmbeddingService: API returned empty or invalid embedding');
        }
    ...
    async generateBatch(texts: string[]): Promise<number[][]> {
        ...
        // Limitar paralelismo a 10 requests simultáneos para respetar rate limits
        const BATCH_SIZE = 10;
        const results: number[][] = [];

        for (let i = 0; i < texts.length; i += BATCH_SIZE) {
            const batch = texts.slice(i, i + BATCH_SIZE);
            const batchResults = await Promise.all(batch.map((t) => this.generate(t)));
