...
export class VisionProcessingConsumer implements OnModuleInit, OnModuleDestroy {
    ...
    constructor(private readonly commandBus: CommandBus) {
        ...
        this.client.on('error', (err) => this.logger.error('Redis Consumer Client Error', err));
    }
    ...
    private async createConsumerGroup() {
        try {
            ...
            this.logger.log(`Created consumer group ${this.consumerGroup} for stream ${this.streamName}`);
        } catch (e: any) {
            ...
            try {
                ...
                if (response && response.length > 0) {
                    ...
                    for (const message of stream.messages) {
                        const { incidentId, employeeId, evidenceUrl } = message.message as any;

                        this.logger.log(`Processing message ${message.id} for incident ${incidentId}`);

                        try {
                            // Fire the application command to process the incident OCR and Validation
                            await this.commandBus.execute(
                                new ProcessIncidentEvidenceCommand(incidentId, employeeId, evidenceUrl),
                            );
