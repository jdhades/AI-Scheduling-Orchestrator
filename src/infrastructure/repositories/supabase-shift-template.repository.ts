...
export class SupabaseShiftTemplateRepository implements IShiftTemplateRepository {
    ...
    async findAllByCompany(companyId: string): Promise<ShiftTemplate[]> {
        ...
        if (error) throw new Error(`ShiftTemplateRepository.findAllByCompany failed: ${error.message}`);
    ...
    async save(template: ShiftTemplate): Promise<void> {
        ...
        if (error) throw new Error(`ShiftTemplateRepository.save failed: ${error.message}`);
    }
    ...
    async delete(id: string, companyId: string): Promise<void> {
        const { error } = await this.supabase
            ...
            .eq('id', id)
            .eq('company_id', companyId);
        if (error) throw new Error(`ShiftTemplateRepository.delete failed: ${error.message}`);
    }
