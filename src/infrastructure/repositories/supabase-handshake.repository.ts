...
export class SupabaseHandshakeRepository implements IHandshakeRepository {
  ...
  async save(
    ...
    const { error } = await this.supabase.from('whatsapp_handshakes').insert({
      id: handshake.id,
    ...
    if (error)
      throw new Error(`HandshakeRepository.save failed: ${error.message}`);
  }
  ...
  async markVerified(handshakeId: string): Promise<void> {
    ...
    if (error)
      throw new Error(
        `HandshakeRepository.markVerified failed: ${error.message}`,
      );
  }
}
