...
export class SupabaseFairnessHistoryRepository implements IFairnessHistoryRepository {
  ...
  async findByWeek(
    ...
    if (error)
      throw new Error(
        `FairnessHistoryRepository.findByWeek failed: ${error.message}`,
      );
  ...
  async upsert(history: FairnessHistoryVO): Promise<void> {
    const { error } = await this.supabase
      .from('fairness_history')
      .upsert(this.toRow(history), { onConflict: 'employee_id,week_start' });
    if (error)
      throw new Error(
        `FairnessHistoryRepository.upsert failed: ${error.message}`,
      );
  }
  ...
  async upsertBatch(histories: FairnessHistoryVO[]): Promise<void> {
    ...
    if (error)
      throw new Error(
        `FairnessHistoryRepository.upsertBatch failed: ${error.message}`,
      );
  }
