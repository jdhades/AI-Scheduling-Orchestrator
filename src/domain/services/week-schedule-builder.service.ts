...
import { SEMANTIC_BLOCKED_ALL } from '../types/scheduling-types';
...
export interface BuilderResult {
   ...
   * mejor opción disponible, este array describe la situación al manager
   * (ej. "X hard policy violations — kept LLM attempt 1, deterministic
   * was worse"). Vacío en el camino feliz.
   */
...
export class WeekScheduleBuilder {
  ...
  async buildWithRetries(params: {
    ...
    signal?: AbortSignal;
    /**
     * Locale del manager — usado para construir warnings en su idioma
    ...
    // que llegan al manager via WhatsApp / dashboard sin más procesado;
    // por eso traducimos acá en vez de pasar IDs y resolver en el front.
    const warn = {
      detFallbackNoLLM: (n: number) =>
        isEs
          ? `Horario tiene ${n} violación(es) hard de policy — generado por fallback determinístico (sin LLM).`
          : `Schedule has ${n} hard policy violation(s) — generated by deterministic fallback (no LLM available).`,
      bestOfTwo: (best: number, first: number) =>
        isEs
          ? `Horario tiene ${best} violación(es) hard de policy — mejor intento LLM (mejoró desde ${first} en el primer intento).`
          : `Schedule has ${best} hard policy violation(s) — best LLM attempt (improved from ${first} on first try).`,
      kepLLM1: (best: number, det: number) =>
        isEs
          ? `Horario tiene ${best} violación(es) hard de policy — se conservó el intento LLM 1 (el determinístico era peor: ${det} violaciones).`
          : `Schedule has ${best} hard policy violation(s) — kept LLM attempt 1 (deterministic fallback was worse: ${det} violations).`,
      detUnsatisfiable: (n: number) =>
        isEs
          ? `Horario tiene ${n} violación(es) hard de policy — el fallback determinístico no pudo satisfacer todas las policies.`
          : `Schedule has ${n} hard policy violation(s) — deterministic fallback couldn't satisfy all policies.`,
    };
    ...
    //   - Intento 1: si V₁=0 → ACEPTAR (camino feliz).
    //   - Intento 2: si V₂=0 → ACEPTAR. Si V₂<V₁ → ACEPTAR + warning.
    //                Si V₂≥V₁ → ir al determinístico, conservando best1.
    ...
    for (
      ...
      if (attempt === 1) {
        ...
        feedback = this.formatFeedback(allHardViolations);
        continue;
      }

      // Intento 2:
      const v1 = best1!.hardCount;
      const v2 = hardCount;
      if (v2 < v1) {
      ...
      // Intento 2 no mejoró — vamos al determinístico.
      this.logger.warn(
        `Intento 2 no mejoró (V₁=${v1}, V₂=${v2}). Pasando a determinístico (conservando intento 1).`,
      );
      break;
    }
    ...
    if (best1 && detCount > best1.hardCount) {
      // El determinístico es peor que el intento 1 del LLM → tomar best1.
      const warning = warn.kepLLM1(best1.hardCount, detCount);
      this.logger.warn(warning);
      return {
        ...best1.candidate,
        softPolicyViolations: best1.softPolicyViolations,
        policyWarnings: [warning],
    ...
    // Determinístico es igual o mejor — tomar determinístico.
    const detWarnings: string[] = [];
    if (detCount > 0) {
      detWarnings.push(warn.detUnsatisfiable(detCount));
    }
    return {
      ...deterministic,
      softPolicyViolations: detEval.softViolations,
      policyWarnings: detWarnings,
      attempts: WeekScheduleBuilder.MAX_LLM_ATTEMPTS + 1,
      fellBackToDeterministic: true,
    };
  }
