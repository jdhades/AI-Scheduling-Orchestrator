  ...
  async buildWithRetries(params: {
    ...
    if (!this.proposer) {
      ...
      const policyWarnings = evalResult.hardViolations.length > 0
        ? [`Schedule has ${evalResult.hardViolations.length} hard policy violation(s) — generated by deterministic fallback (no LLM available).`]
        : [];
    ...
    for (let attempt = 1; attempt <= WeekScheduleBuilder.MAX_LLM_ATTEMPTS; attempt++) {
      ...
      if (v2 < v1) {
        ...
        const warning = `Schedule has ${v2} hard policy violation(s) — best LLM attempt (improved from ${v1} on first try).`;
    ...
    if (best1 && detCount > best1.hardCount) {
      ...
      const warning = `Schedule has ${best1.hardCount} hard policy violation(s) — kept LLM attempt 1 (deterministic fallback was worse: ${detCount} violations).`;
    ...
    if (detCount > 0) {
      detWarnings.push(
        `Schedule has ${detCount} hard policy violation(s) — deterministic fallback couldn't satisfy all policies.`,
      );
    }
    return {
      ...deterministic,
