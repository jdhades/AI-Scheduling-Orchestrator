...
export class ConflictResolutionService {
  ...
  resolve(
    ...
    // 1. Legal rules
    const legalResult = this.checkLegalRules(
      shift,
      assignedShiftsThisWeek,
      weeklyHistory,
    );
    if (!legalResult.valid) return legalResult;

    // 2. Skill constraints
    const skillResult = this.skillPolicy.canWork(
      employee,
      shift,
      companySkills,
      shift.startTime,
    ...
    if (
      shift.undesirableWeight.isHeavy() &&
      !this.thresholdGuard.canReceiveHeavyShift(score.value)
    ) {
      const alt = this.findAlternative(candidates, shift, companySkills);
      return {
        valid: false,
  ...
  private checkLegalRules(
    ...
    const hasInsufficientRest = assignedThisWeek.some((existing) => {
      const gapBefore =
        (shift.startTime.getTime() - existing.endTime.getTime()) /
        (1000 * 60 * 60);
      const gapAfter =
        (existing.startTime.getTime() - shift.endTime.getTime()) /
        (1000 * 60 * 60);
      return (
        (gapBefore >= 0 &&
          gapBefore < ConflictResolutionService.MIN_REST_HOURS) ||
  ...
  private findAlternative(
    ...
    return candidates.find((c) => {
      const result = this.skillPolicy.canWork(
        c,
        shift,
        companySkills,
        shift.startTime,
