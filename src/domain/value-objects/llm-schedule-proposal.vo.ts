...
export class LLMScheduleProposalVO {
    ...
    static fromLLMResponse(raw: string): LLMScheduleProposalVO {
        ...
        if (!jsonMatch) {
            throw new Error('LLMScheduleProposal: no JSON block found in LLM response');
        }
        ...
        if (!Array.isArray(parsed.assignments)) {
            throw new Error('LLMScheduleProposal: "assignments" field must be an array');
        }

        const proposals = parsed.assignments.map((item: unknown, idx: number) => {
            const a = item as Record<string, unknown>;
            if (typeof a.shiftId !== 'string') {
                throw new Error(`LLMScheduleProposal: assignment[${idx}].shiftId must be a string`);
            }
            if (typeof a.employeeId !== 'string') {
                throw new Error(`LLMScheduleProposal: assignment[${idx}].employeeId must be a string`);
            }
