...
export class IncidentValidationPolicy {
    ...
    static validateCertificateParams(
        ...
        if (!parsedData.patient_name || !parsedData.issue_date || !parsedData.rest_days) {
            throw new DomainError('Missing essential fields in the certificate (Name, Date, or Rest Days)');
        }
        ...
        const matchCount = employeeTokens.filter(token => patientNameLower.includes(token)).length;

        // If less than half the name tokens match, flag as suspicious
        if (matchCount < employeeTokens.length / 2) {
            throw new DomainError(`Name mismatch: Certificate says '${parsedData.patient_name}', but employee is '${employeeName}'`);
        }
