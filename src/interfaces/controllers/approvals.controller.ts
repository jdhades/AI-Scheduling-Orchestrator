...
export class ApprovalsController {
  ...
  async pendingCounts(@CurrentCompany() companyId: string): Promise<PendingCountsDTO> {
    const since = new Date(Date.now() - 14 * 86_400_000).toISOString();
    const head = (table: string) =>
      this.supabase.from(table).select('id', { count: 'exact', head: true }).eq('company_id', companyId);

    const [corrections, swaps, dayoffs, incidents, absences] = await Promise.all([
      ...
      head('incidents').eq('status', 'PENDING'),
      head('absence_reports').is('deleted_at', null).gte('reported_at', since),
    ]);
    ...
    return { corrections: c, swaps: s, dayoffs: d, incidents: i, absences: a, total: c + s + d + i };
  }
}
