...
export class InstantiateWeekHandler {
  ...
  async execute(
    command: InstantiateWeekCommand,
  ): Promise<InstantiateWeekResult> {
    ...
    if (weekMondayUtc.getUTCDay() !== 1) {
      throw new Error(
        `weekStart must be a Monday. Got: ${weekStart} (weekday ${weekMondayUtc.getUTCDay()})`,
      );
    }
    ...
    if (templates.length === 0) {
      this.logger.warn(
        `No active shift templates found for company ${companyId}`,
      );
      return { generated: 0, shifts: [] };
    }

    this.logger.log(
      `Instantiating ${templates.length} templates for week ${weekStart}`,
    );

    // Instantiate each template into a concrete Shift
    const newShifts: Shift[] = templates.map((t) =>
      t.instantiateForWeek(weekMondayUtc),
    );
    ...
    if (failures.length > 0) {
      this.logger.error(
        `${failures.length}/${newShifts.length} shifts failed to save`,
        failures,
      );
    }

    const savedShifts = newShifts.filter(
      (_, i) => results[i].status === 'fulfilled',
    );

    this.logger.log(
      `✅ Instantiated ${savedShifts.length} shifts for week ${weekStart}`,
    );
