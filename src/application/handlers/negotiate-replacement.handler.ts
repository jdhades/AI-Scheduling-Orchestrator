...
export class NegotiateReplacementHandler implements ICommandHandler<NegotiateReplacementCommand> {
  ...
  async execute(command: NegotiateReplacementCommand): Promise<void> {
    ...
    this.logger.log(
      `Initiating WhatsApp negotiation for Incident ${incidentId} with Employee ${candidateEmployeeId} for Shift ${shiftId}`,
    );

    // The command also passed companyId
    const candidate = await this.employeeRepository.findById(
      candidateEmployeeId,
      command.companyId,
    );
    ...
    if (!candidate) {
      this.logger.error(
        `Candidate ${candidateEmployeeId} not found. Aborting negotiation.`,
      );
      return;
    }
    ...
    this.logger.log(
      `[TWILIO MOCK] Message Sent to ${candidate.id}: "Hola, ¿puedes cubrir el turno ${shiftId} de urgencia hoy? (Responde SÍ o NO)"`,
    );
    ...
    this.logger.log(
      `Message successfully queued for Twilio Delivery. Waiting for Candidate response...`,
    );
  }
}
