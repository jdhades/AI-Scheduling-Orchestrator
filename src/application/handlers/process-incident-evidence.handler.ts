...
export class ProcessIncidentEvidenceHandler implements ICommandHandler<ProcessIncidentEvidenceCommand> {
  constructor(
  ...
  async execute(command: ProcessIncidentEvidenceCommand): Promise<void> {
    ...
    const employeeObj = await this.employeeRepository.findById(
      incident.employeeId,
      incident.companyId,
    );
    if (!employeeObj) {
    ...
    const { rawText, confidence } =
      await this.ocrService.extractTextFromDocument(mediaUrl);
    ...
    if (incident.status === 'pending_validation') {
      try {
        const parsedData =
          await this.llmParsingService.parseMedicalCertificate(rawText);

        // Phase 9 Validation Policy (Similarity check)
        const employeeMockName = (employeeObj as any).name || 'John Doe';
        IncidentValidationPolicy.validateCertificateParams(
          parsedData,
          employeeMockName,
        );
