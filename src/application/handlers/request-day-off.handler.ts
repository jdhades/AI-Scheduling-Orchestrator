...
export class RequestDayOffHandler implements ICommandHandler<RequestDayOffCommand> {
    ...
    async execute(command: RequestDayOffCommand): Promise<{ message: string }> {
        ...
        const assignments = await this.shiftRepo.findAssignmentsByEmployee(
            employeeId, companyId, weekStart,
        );
        const shifts = await this.shiftRepo.findByCompanyAndWeek(companyId, weekStart);
        ...
        if (hasShiftOnDate) {
            return {
                message: `⚠️ Ya tienes un turno asignado el ${date}. ` +
        ...
        return { message: `✅ Tu solicitud de día libre para el ${date} fue enviada al manager para aprobación.` };
    }
