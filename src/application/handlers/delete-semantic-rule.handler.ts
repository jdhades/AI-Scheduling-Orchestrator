...
export class DeleteSemanticRuleHandler
    implements ICommandHandler<DeleteSemanticRuleCommand, { deleted: boolean }> {
    ...
    async execute(command: DeleteSemanticRuleCommand): Promise<{ deleted: boolean }> {
        // Verificar que la regla existe y pertenece a la empresa
        const rule = await this.ruleRepository.findById(command.ruleId, command.companyId);
