...
export class IncidentValidatedHandler
    implements IEventHandler<IncidentValidatedEvent> {
    ...
    async handle(event: IncidentValidatedEvent) {
        ...
        const employeeShifts = await this.shiftRepository.findByCompanyAndWeek(incident.companyId, event.payload.startDate);
        ...
        const myShifts = employeeShifts.filter(s => {
            ...
            if (assignments && Array.isArray(assignments.assignments)) {
                return assignments.assignments.some((a: any) => a.employeeId === event.employeeId);
            }
            return false;
        });
        ...
        if (affectedShiftsCount.length > 0) {
            ...
            const availableColleagues = await this.employeeRepository.findAllByCompany(incident.companyId);
        ...
        if (incident.status === 'replacement_assigned') {
            incident.resolveIncident('Auto-Repair assigned all replacements internaly.');
        }
